...
void USKGLightLaserComponent::RegisterAsInfraredDevice()
{
	if (bHasInfraredMode)
	{
		if (USKGShooterWorldSubsystem* Subsystem = GetWorld()->GetSubsystem<USKGShooterWorldSubsystem>())
		{
...
void USKGLightLaserComponent::UnregisterAsInfraredDevice()
{
	if (bHasInfraredMode)
	{
		GetWorld()->GetSubsystem<USKGShooterWorldSubsystem>()->UnregisterInfraredDevice(this);
	}
}
...
void USKGLightLaserComponent::OnRep_LaserState()
{
	if (LaserMesh)
	{
		...
		const bool bPlayerInfraredOn = GetWorld()->GetSubsystem<USKGShooterWorldSubsystem>()->IsInfraredModeEnabledOnPlayer();
