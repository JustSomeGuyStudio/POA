...
void USKGShooterFrameworkAnimInstance::SetupShooterPawnComponent()
{
	...
	{
		ShooterPawnComponent->OnHeldActorSet.AddDynamic(this, &USKGShooterFrameworkAnimInstance::OnHeldActorSet);
...
void USKGShooterFrameworkAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	{
		bUseFirstPerson = bIsLocallyControlled && ShooterPawnComponent->UseFirstPersonProceduralsAsLocal();
...
void USKGShooterFrameworkAnimInstance::SetupInitialData()
{
	...
	OffHandIKLocation = ProceduralAnimData.OffHandIKOffset.GetLocation();
...
{
	CameraOffsetLocation = Offset.GetLocation();
...
void USKGShooterFrameworkAnimInstance::SetRecoilData()
{
	if (bPerformingRecoil)
	{
		...
		{
			ControlRotationRecoilTarget = ControlRotationCurve->GetVectorValue(ElapsedTimeSinceStart) * RecoilControlRotationMultiplier;
		}
		if (const UCurveVector* LocationCurve = ProceduralAnimData.RecoilSettings.LocationCurve)
		{
			RecoilLocationTarget = LocationCurve->GetVectorValue(ElapsedTimeSinceStart) * RecoilLocationMultiplier;
		}
		if (const UCurveVector* RotationCurve = ProceduralAnimData.RecoilSettings.RotationCurve)
		{
			RecoilRotationTarget = RotationCurve->GetVectorValue(ElapsedTimeSinceStart) * RecoilRotationMultiplier;
		}
	}
}
...
{
	if (bUseProceduralSpineUpDown && !ProceduralAnimData.bInFreeLook && (OldSpineControlRotation != ControlRotation || SpineRotationUpDown != SpineRotationToInterpTo))
	{
...
{
	if (!SightOffsetLocation.Equals(ProceduralAnimData.AimOffset.GetLocation()) || !SightOffsetRotation.Equals(ProceduralAnimData.AimOffset.Rotator()))
	{
		if (ProceduralAnimData.CycleAimingPointSettings.bUseSpringInterpForCyclingAimingPoint)
		{
			...
			SightOffsetLocation = UKismetMathLibrary::VectorSpringInterp(SightOffsetLocation, ProceduralAnimData.AimOffset.GetLocation(), AimingLocationSpringState, SpringSettings.LocationStiffness, SpringSettings.LocationCriticalDampingFactor, DeltaSeconds, SpringSettings.LocationMass, SpringSettings.LocationTargetVelocityAmount);
		...
		{
			SightOffsetLocation = FMath::VInterpTo(SightOffsetLocation, ProceduralAnimData.AimOffset.GetLocation(), DeltaSeconds, ProceduralAnimData.AimOffsetInterpSpeed);
...
void USKGShooterFrameworkAnimInstance::HandleMovementSway(float DeltaSeconds)
{
	if (ProceduralAnimData.MovementSwaySettings.bUseMovementSway)
	{
		...
		MovementSwayCurveLocation *= MovementSwayLocationMultiplier;
		MovementSwayCurveRotation *= MovementSwayRotationMultiplier;
	...
	if (bPerformingRecoil)
	{
		...
		if (bIsLocallyControlled && !ControlRotationRecoilTarget.Equals(FVector::ZeroVector))
		{
			if (APawn* Pawn = TryGetPawnOwner())
			{
				...
				const FRotator ControlRotationRecoilTargetRot = FRotator(ControlRotationRecoilTarget.X, ControlRotationRecoilTarget.Y, ControlRotationRecoilTarget.Z) * Delta;
...
void USKGShooterFrameworkAnimInstance::HandleFirearmCollision(float DeltaSeconds)
{
	if (ProceduralAnimData.FirearmCollisionSettings.bUseFirearmCollision)
	{
		...
		const float MuzzleToRootDistance = MuzzleRelativeTransform.GetLocation().Size();
		...
		FirearmCollisionTraceEnd = EndTransform.GetLocation();
