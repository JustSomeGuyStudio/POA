...
void USKGFirearmComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(USKGFirearmComponent, MuzzleComponents, ParamsOwnerOnly);
	...
	if (!bIsPointAiming)
	{
		...
		if (ProceduralAnimComponents.Num())
		{
			...
			for (int32 i = 0; i < ProceduralAnimComponents.Num(); ++i)
			{
				...
				if (TempProceduralAnimComponent && TempProceduralAnimComponent->GetCanAim() && TempProceduralAnimComponent != CurrentProceduralAnimComponent)
				{
			...
			if (NewProceduralComponent && !NewProceduralComponent->GetCanAim())
			{
		...
		if (NewProceduralComponent && CurrentProceduralAnimComponent != NewProceduralComponent)
		{
			CurrentProceduralAnimComponent = NewProceduralComponent;
			if (CurrentOpticComponent)
			{
				CurrentOpticComponent->StoppedAiming();
			}
			bNewOpticComponent = SetOpticComponent();
...
void USKGFirearmComponent::SetBestProceduralAnimComponent()
{
	if (!CurrentProceduralAnimComponent || (CurrentProceduralAnimComponent == FirearmProceduralAnimComponent))
	{
		...
		for (int32 i = 0; i < ProceduralAnimComponents.Num(); ++i)
		{
			...
			if (ProceduralAnimComponent && ProceduralAnimComponent->GetCanAim())
			{
